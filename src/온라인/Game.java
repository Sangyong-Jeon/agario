...
public class Game extends JFrame implements MouseMotionListener { // JFrame이라고 불리는 클래스 상속받고, MouseMotionListener 구현함.
	...
	public Game() {
		...
		Thread cc = new Thread(new Create(), "두번째");
		cc.run();
...
//		pane.setBounds(300, 0, 1600, height);
...
//		jlp.add(pane, new Integer(1));
	...
	public void draw() {
		...
		for (Cell cell : Cell.cells) { // 내 좌표값 우측 상단에 보여주기
			if (cell.name.equals("Bruce")) {
				...
				bbg2.drawString(pos, (Game.width - pos.length() * pos.length() - 250), 10);
			}
		}
		lb.Draw(bbg2);
	...
	public void mouseMoved(MouseEvent e) { // 마우스가 클릭되지 않고 이동하는 경우 호출
		...
		for (Cell cell : Cell.cells) {
			if (cell.name.equals("Bruce")) {
				cell.getMouseX((int) (e.getX() / cam.sX + cam.x) + 100);
